    ...
    public class PeriodRepository : AbstractRepository<Period>
    {
        ...
        public override bool Insert(Period item)
        {
            using(IDbConnection db = new SqlConnection(ConnectionString))
            {
                ...
                string query = "INSERT INTO PERIOD (START_DATE, END_DATE, MONTHS, NAME) VALUES (@Startdate, @EndDate, @Months, @Name)";
        ...
        public override bool Update(Period item)
        {
            using(IDbConnection db = new SqlConnection(ConnectionString))
            {
                ...
                string query = "UPDATE PERIOD SET START_DATE = @StartDate, END_DATE = @EndDate, NAME = @Name WHERE ID = @Id";
                var result = db.Execute(query, item);
                db.Close();
                return result > 0;
            }
